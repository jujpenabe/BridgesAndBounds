...
# create empty array list of AudioStreamPlayer "_audioStreamPlayers"
...
func _ready() -> void:
	...
	if child is AudioStreamPlayer:
		...
		for i in range(count):
			...
			_audioStreamPlayers.append(newChild)
			add_child(newChild)
...
func play_sound() -> void:
	...
	if !_audioStreamPlayers[_next].is_playing():
		# change the volume of the next audio stream player
		_audioStreamPlayers[_next].volume_db = _current_volume
		# play next+1 audio stream player
		_audioStreamPlayers[_next+1].play()
		_is_playing = true
...
func set_volume_db(volume_db:float) -> void:
	...
	_audioStreamPlayers[_next].volume_db = volume_db
